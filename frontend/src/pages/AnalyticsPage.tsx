...
import { BarChart3, Download, Filter, ShieldCheck } from 'lucide-react'
...
function DistributionChart({
  ...
  const option: EChartsOption = chartType === 'donut'
    ...
    : horizontal
      ? {
        ...
        series: [{ type: 'bar', data: values, itemStyle: { color: '#0f766e', borderRadius: [0, 6, 6, 0] } }],
      }
      : {
        tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
        ...
        series: [{ type: 'bar', data: values, itemStyle: { color: '#0f766e', borderRadius: [6, 6, 0, 0] } }],
      }
  ...
  return (
    <article className="panel analytics-chart">
      ...
      ) : <ClinicalChart height={230} option={option} />}
    </article>
  )
}
...
export default function AnalyticsPage() {
  ...
  return <section className="page-grid">
    ...
    {distributionQuery.isLoading ? <LoadingState label="Loading distributions" /> : <section className="analytics-chart-grid"><DistributionChart title="Stage distribution" items={distributionQuery.data?.stage ?? []} /><DistributionChart title="Biomarker frequency" items={distributionQuery.data?.biomarker ?? []} horizontal defaultTopN={10} /><DistributionChart title="Treatment patterns" items={distributionQuery.data?.treatment ?? []} horizontal defaultTopN={10} /><DistributionChart title="Recorded responses" items={distributionQuery.data?.response ?? []} defaultChartType="donut" /></section>}
